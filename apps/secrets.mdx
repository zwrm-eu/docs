...
<Tip>For non-sensitive config, use the `[env]` section in [zwrm.toml](/apps/configuration) instead.</Tip>
...
The preferred form is `--stdin`, which reads the value from standard input so it never appears in argv or your shell history:

```bash
printf '%s' "$API_KEY" | zwrm secrets set API_KEY --stdin
```

With `--stdin`, pass only the secret name.
