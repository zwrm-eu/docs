...
Both tools need the agent's `memories:write` [capability](/agents/capabilities), which is in the default set. Narrow it out, or turn off the agent's management access, and the agent can still read its memory but no longer change it.
