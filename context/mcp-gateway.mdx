...
In the dashboard, the gateway page has a **Quick connect** grid of branded cards (Sentry, GitHub, Mintlify) that pre-fills the registration form so you only supply credentials.
...
- `--credential-mode <upstream>=<mode>` and `--bound-grant <upstream>=<type>:<id>` set which account calls act as on a per-user OAuth upstream. See [Connector identity](/context/credentials#resolution-modes).
