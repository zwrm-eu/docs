...
- `--bound-grant <upstream>=<type>:<id>` pins one pre-consented account for every caller on a per-user OAuth upstream. By default each caller acts as their own account — see [Connector identity](/context/credentials).
