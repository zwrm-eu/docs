...
- **Agents** — authorize the agent from its agent page (or `zwrm mcp connect ms365 --agent <agent>`); the agent then acts as its own connected account. Consent with a purpose-scoped service account, not a personal one — the agent gets that account's entire reach.
