...
| `--model` | string | agent default | `opus` or `sonnet` |
...
- **Files** — anything the agent staged in `~/outputs/` is snapshotted onto the run at finalize.
- **Messages** — anything sent via `send_message` lands in the agent's [Inbox](/agents/messaging) and your configured channels.
